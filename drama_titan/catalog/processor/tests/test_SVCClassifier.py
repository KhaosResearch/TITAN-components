...
from drama_titan.catalog.processor.ml.SVCClassifierTrain import execute
from drama_titan.catalog.processor.tests import RESOURCES
from drama_titan.model import MlModelSVC


class SVMClassifierTrainTestCase(unittest.TestCase):
    def setUp(self) -> None:
        task_id, task_name = "drama", "test_SVMClassifierTrain"
    ...
    def test_integration(self):
        ...
        self.assertTrue(Path(self.pcs.storage.local_dir, "svc_model.bin.pickle").is_file())

        # assert output data is valid
        self.assertEqual(data.keys(), {"output", "resource"})
        self.assertIs(type(data["output"]), MlModelSVC)
